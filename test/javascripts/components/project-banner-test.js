...
module("Projects | Component | ProjectBanner", function (hooks) {
  ...
  test("when banner is rendered", async function (assert) {
    ...
    assert.dom(".project-banner__wrapper__link").exists("it shows a link to the corresponding category");
    assert.dom(".project-banner__wrapper__link").hasText('Foo');
    assert.dom(".project-banner__wrapper__link").hasAttribute('href', '/c/foo/1');
  });
});